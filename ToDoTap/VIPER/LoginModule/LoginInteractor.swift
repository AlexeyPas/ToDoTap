...
extension LoginInteractor: LoginInteractorInput{
    func register(data: RegisterData) {
        Auth.auth().createUser(withEmail: data.email, password: data.password) { [weak self] (user, error) in
            ...
            self.db.collection("users").document(authContext.id).collection("todayTask").document("Напишите задачи на сегодня. Например: Убраться в комнате").setData(["isCheck" : false]){ err in
                if let err = err {
                    print("Error adding document: \(err)")
                    self.presenter?.gotError(.error)
                    return
            ...
            self.db.collection("users").document(authContext.id).collection("tomorrowTask").document("Напишите задачи на завтра. Например: Помыть машину").setData(["isCheck" : false]){ err in
                if let err = err {
                    print("Error adding document: \(err)")
                    self.presenter?.gotError(.error)
                    return
            ...
            self.db.collection("users").document(authContext.id).collection("weekTask").document("Напишите задачи на эту неделю. Например: Съезить к маме").setData(["isCheck" : false]){ err in
                if let err = err {
                    print("Error adding document: \(err)")
                    self.presenter?.gotError(.error)
                    return
