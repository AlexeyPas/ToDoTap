...
//        bgView.backgroundColor = UIColor(displayP3Red: 9.0/255.0, green: 33.0/255.0, blue: 47.0/255.0, alpha: 1.0).withAlphaComponent(0.7)
    ...
    init(_ output:AccountViewOutput){
        self.output = output
...
private extension AccountViewController{
    @objc
    func onSubmitTap(){
        let data: AccountAndPasswordData = getAccountData()
        output.Account(data: data)
    }
    @objc
    func onRegisterTap(){
        //let data: AccountAndPasswordData = getAccountData()
        //output.register(type: .AccountAndPassword, data: data)
    }
    @objc
    ...
    func getAccountData() -> AccountAndPasswordData{
        let data: AccountAndPasswordData = AccountAndPasswordData(Account: textFieldView1.textField.text ?? "", password: textFieldView2.textField.text ?? "")
        return data
    }
