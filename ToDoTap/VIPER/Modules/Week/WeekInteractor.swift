...
final class WeekInteractor {
	weak var output: WeekInteractorOutput?
...
extension WeekInteractor: WeekInteractorInput {
    func fetchTasks() {
        guard let userID = Auth.auth().currentUser?.uid else { return }
        let docRef = self.db.collection("users").document(userID).collection("weekTask")
    ...
    func loadDate(task: String) {
        guard let userID = Auth.auth().currentUser?.uid else { return }
        let washingtonRef = db.collection("users").document(userID).collection("weekTask")
